...
void Evolver<Ind>::fileSettings(const std::string& file, const std::string& prefix){
	...
	if(fp(prefix+"averageStalledTol",averageStalledTol) && fp(prefix+"averageStalledMax",averageStalledMax))
		addCriteria(GA::StopReason::AverageStalled, {averageStalledTol, (double)averageStalledMax});

	double bestStalledTol;
	unsigned bestStalledMax;
	if(fp(prefix+"bestStalledTol",bestStalledTol) && fp(prefix+"bestStalledMax",bestStalledMax))
		addCriteria(GA::StopReason::BestStalled, {bestStalledTol, (double)bestStalledMax});


	double averageRelativeStalledTol;
	unsigned averageRelativeStalledMax;
	if(fp(prefix+"averageRelativeStalledTol",averageRelativeStalledTol) && fp(prefix+"averageRelativeStalledMax",averageRelativeStalledMax))
		addCriteria(GA::StopReason::AverageRelativeStalled, {averageRelativeStalledTol, (double)averageRelativeStalledMax});

	double bestRelativeStalledTol;
	unsigned bestRelativeStalledMax;
	if(fp(prefix+"bestRelativeStalledTol",bestRelativeStalledTol) && fp(prefix+"bestRelativeStalledMax",bestRelativeStalledMax))
		addCriteria(GA::StopReason::BestRelativeStalled, {bestRelativeStalledTol, (double)bestRelativeStalledMax});
...
void Evolver<Ind>::finish(StopReason stop) const{
	if(verbose) StopCriteria::printStopReason(stop);
...
inline void Evolver<Ind>::printGen() const{
	if(verbose){
		printf("Generation [%3d]; Best=%.3e; Average=%.3e; Best genes=%s; ",
			generationStep
...
template <typename Ind>
unsigned Evolver<Ind>::selectParent(const VecD& fitnessCumulative, int other) const
{
...
unsigned Evolver<Ind>::findElite(Population<Ind> *offspring){
	if(population->speciation_threshold < 0. || dissimilarity==nullptr){ //no speciation - same as below, but faster
		...
		if(eliteCount) std::nth_element(population->species[0]->pop.begin(), population->species[0]->pop.begin()+eliteCount-1, population->species[0]->pop.end(), 
			Species<Ind>::compareIndividuals);
		...
		FORV(i,numSpecies){
			if((population->species[i]->gensNoImprovement > oldAge || 
				(generationStep>1 && generationStep%obliterationRate==0 && i==population->worseSpecies)) && i!=population->bestSpecies)
				continue;
			...
			if(!eliteCountPerSpecies) continue;
			...
			// std::partial_sort(population->species[i]->pop.begin(), population->species[i]->pop.begin()+eliteCountPerSpecies, population->species[i]->pop.end(), 
				// Species<Ind>::compareIndividuals);
			std::nth_element(population->species[i]->pop.begin(), population->species[i]->pop.begin()+eliteCountPerSpecies-1, population->species[i]->pop.end(), 
				Species<Ind>::compareIndividuals);
			// std::sort(population->species[i]->pop.begin(), population->species[i]->pop.end(), 
				// Species<Ind>::compareIndividuals);			
...
void Evolver<Ind>::nullifyElite(){
	if(population->speciation_threshold < 0. || dissimilarity==nullptr){ //no speciation - same as below, but faster
		...
		FORV(i,numSpecies){
			...
			if(!eliteCountPerSpecies) continue; //unecessary, cause would skip for anyways, but here just to match the code in 'findElite'

			FOR(ind, eliteCountPerSpecies) population->species[i]->pop[ind] = nullptr; //directly copied to offspring, so can be made null here
		}
	}
}
...
VecD Evolver<Ind>::calculateExpectedOffspring(unsigned eliteDone) const{
	...
	FORV(i, offspring.size()){
		// std::cout << "SPECIES ID=" << population->species[i]->id << "; FITSUM=" << population->species[i]->fitnessSum << std::endl;
		offspring[i] = (unsigned)((total*population->species[i]->fitnessSum)/population->totalFitnessSum);
		partial_sum += offspring[i];
	}

	//distribute the rest (starting in best species just in case there's only 1 left)
	unsigned i=population->bestSpecies;
	while(partial_sum < total){
		++offspring[i];
		++partial_sum;
		++i;
		if(i==population->numSpecies()) i=0;
	}
...
template <typename Ind>
void Evolver<Ind>::eliminateWeak(){
	if(survivalThreshold==0.) return;
	...
	//This has to make sure that in the end there are at least 1! Don't forget that there might be only one during reproduction
	FORV(s,population->numSpecies()){
		unsigned eliteCountPerSpecies = (s<breakPoint ? eliteCountPerSpeciesInt+1 : eliteCountPerSpeciesInt);
		eliteCountPerSpecies = std::min(eliteCountPerSpecies, population->species[s]->size());

		unsigned eliminate = population->species[s]->size() * survivalThreshold;
		eliminate = std::min( eliminate , std::max(population->species[s]->size()-2, (unsigned)0) );			//at least one survives
		eliminate = std::min( eliminate , population->species[s]->size() - eliteCountPerSpecies );  //at least elite survives

		if(!eliminate) continue;

		//test both partial_sort and nth_element (to see what is better later)
		std::nth_element(population->species[s]->pop.begin(), population->species[s]->pop.end() - eliminate, population->species[s]->pop.end(), 
			Species<Ind>::compareIndividuals);
		// std::sort(population->species[s]->pop.begin(), population->species[s]->pop.end(), 
			// Species<Ind>::compareIndividuals);
		//now that the worse 'eliminate' are in the end of the vector, delete them
		for(unsigned i=population->species[s]->size()-1, count=0; count<eliminate; --i, ++count)
			population->species[s]->clear(i);

		population->species[s]->pop.erase(population->species[s]->size()-eliminate, population->species[s]->size()-1);
	}
}

template <typename Ind>
void Evolver<Ind>::generation(){
	...
	eliminateWeak(); //it's ok to eliminate before doing 'calculateExpectedOffspring', cause the full sum of fitnesses is already saved
	...
	FORV(s,population->numSpecies()){
		...
		default(none) \
		shared(s, population, crossoverProb, mutateProb, expectedOffspring, offspring, generator, std::cout, speciesCumulativeSum)
		for(unsigned ind = 0; ind<expectedOffspring[s]; ++ind){

			unsigned parent1 = selectParent(population->species[s]->fitnessCumulative);

			Individual<Ind> *child;

			if(generator() <= crossoverProb){
				if(population->speciation_threshold >= 0. && dissimilarity!=nullptr && generator() < interSpeciesProb){
					unsigned speciesSelected = selectParent(speciesCumulativeSum, s); //select a species other than 's'
					unsigned parent2 = selectParent(population->species[speciesSelected]->fitnessCumulative); //select an Individual from that species
					// std::cout << "IN SPECIES " << s << " (" << parent1 << "|" << population->species[s]->size() << 
						// "); MATING WITH SPECIES " << speciesSelected << " (" << parent2 << "|" << population->species[speciesSelected]->size() << ")" << std::endl;
					child = new Individual<Ind>(crossover(population->species[s]->pop[parent1]->I,population->species[speciesSelected]->pop[parent2]->I,
						population->species[s]->pop[parent1]->fitness_orig,population->species[speciesSelected]->pop[parent2]->fitness_orig));
				}
				else{
					unsigned parent2 = selectParent(population->species[s]->fitnessCumulative, parent1);
					// std::cout << "PARENTS " << parent1 << " && " << parent2 << std::endl;
					child = new Individual<Ind>(crossover(population->species[s]->pop[parent1]->I,population->species[s]->pop[parent2]->I,
						population->species[s]->pop[parent1]->fitness_orig,population->species[s]->pop[parent2]->fitness_orig));
				}
			}
			else
				child = new Individual<Ind>(*population->species[s]->pop[parent1]);
...
StopReason Evolver<Ind>::updateFitness(){
	...
	FORV(s,population->numSpecies()){
		...
		population->species[s]->bestFitness  = -std::numeric_limits<double>::infinity();
		...
		if( population->species[s]->pop[population->species[s]->bestRank]->fitness_orig > population->species[s]->bestFitnessEver_orig * (obj==MAXIMIZE ? 1. : -1.) ){
			population->species[s]->bestFitnessEver_orig = population->species[s]->pop[population->species[s]->bestRank]->fitness_orig;
	...
	FORV(s,population->numSpecies()){
		FORV(i, population->species[s]->size()){
			...
			if(ind!=nullptr){
				...
				//this makes the champion not necessarily the one with the best fitness
				ind->fitness /= population->species[s]->size();
				if((population->species[s]->gensNoImprovement > oldAge ||
					(generationStep>1 && generationStep%obliterationRate==0 && s==population->worseSpecies)) && s!=population->bestSpecies)
	...
	return criteria.update(generationStep, population->oldBest, newBest, population->oldAverage, newAverage);
}

template <typename Ind>
void Evolver<Ind>::checkSettings() const{
	if(create==nullptr)
...
void Evolver<Ind>::addIndividual(Population<Ind> *pop, Individual<Ind> *I, int species){
	...
	int chosen = (species >= 0 ? species : 0); //int because there's a 'for' below which is reversed and ends in -1
	//automatically add to chosen species - used for elite members, not to have to calculate dissimilarity again
	if(species >= 0){
		chosen = pop->addSpecies(population->species[species]->id,
			population->species[species]->age,
			population->species[species]->gensNoImprovement,
			population->species[species]->bestFitnessEver_orig);
	}
	else if(species < 0 && pop->speciation_threshold >= 0. && dissimilarity!=nullptr){ //no speciation
		unsigned s;
		for(s=0; s<population->numSpecies(); ++s)
			if( dissimilarity( I->I , population->species[s]->pop[0]->I ) <= pop->speciation_threshold ) break;

		if(s==population->numSpecies()){
			//run throw pop species which are not present in population and check for compatibilities not checked yet
			//(of the new species that pop has)
			for( chosen=pop->numSpecies()-1; chosen>=0 && pop->species[chosen]->id > population->last_id; --chosen )
			...
			if( chosen<0 || pop->species[chosen]->id <= population->last_id )
				chosen = pop->addSpecies(std::max(population->last_id, pop->last_id)+1); //new Species
		}
		else{
			// printf("%.1e ", dissimilarity( I->I , population->species[s]->pop[0]->I));
			chosen = pop->addSpecies(population->species[s]->id,
				population->species[s]->age,
				population->species[s]->gensNoImprovement,
