...
template <typename Ind> class Evolver : public Parameters
{
  public:
    Evolver(const std::string &file, const std::string &prefix = "");
    Evolver(unsigned _populationSize, double _speciation_threshold = -1.,
            bool _verbose = true); // negative threshold means no speciation
    ~Evolver();
    ...
    // same as above
    inline virtual StopReason start(unsigned _eliteCount, double _crossoverProb,
                                    double _mutateProb)
    {
        eliteCount = _eliteCount;
        crossoverProb = _crossoverProb;
        mutateProb = _mutateProb;
        return start();
    }
    inline virtual StopReason step(unsigned _eliteCount, double _crossoverProb,
                                   double _mutateProb)
    {
        eliteCount = _eliteCount;
        crossoverProb = _crossoverProb;
    ...
    inline virtual void evolve(unsigned _eliteCount, double _crossoverProb,
                               double _mutateProb)
    {
        eliteCount = _eliteCount;
        crossoverProb = _crossoverProb;
    ...
    inline void setCrossover(Ind (*func)(const Ind &, const Ind &, double fit1,
                                         double fit2));
    inline void setMutate(void (*func)(Ind &, const Evolver<Ind> *));
    inline void setEvaluate(double (*func)(const Ind &, const Evolver<Ind> *),
                            Objective);
    ...
    inline unsigned getPopulationSize() const;

    inline void addCriteria(StopReason stop, const VecD &params)
    {
        criteria.addCriteria(stop, params);
    }

  protected:
    virtual unsigned selectParent(
        const VecD &fitness_cumulative,
        int other = -1) const; // to be replaced by similarity selection in NEAT
  ...
  private:
    ...
    void addIndividual(Population<Ind> *pop, Individual<Ind> *I,
                       int species = -1);
};

template <typename Ind> inline void mutate_default(Ind &, const Evolver<Ind> *)
{
    return;
}
template <typename Ind> inline std::string toString_default(const Ind &)
{
    return "";
}
// template <typename Ind> inline double
// dissimilarity_default(const Ind&, const Ind&) 	{ return generator(); }
template <typename Ind>
inline bool printPopulation_default(const Population<Ind> *pop)
{
    pop->print();
    return true;
} // to use if needed
