...
#define FORV3(IDX, INIT, SZ) for(unsigned IDX = INIT, MAX=SZ; IDX < MAX; ++IDX)
...
//there is still some redefinitions in initialization (since 'resize' is being used before setting values and 'resize' sets the vector the default T())
//but it's because I found some problems in using reserve and push_back
...
public:
	...
	inline Vec(const unsigned, const T*);//can't have constant, not to conflict with the constructor above
	...
	//use updated constructor above (with initializer list {...}) instead. Keeping the one below just for backward compatibility
    ...
    inline Vec(const int n, const T x, T2...rest){setEntries(n,x,rest...);}

	template <class T2> operator Vec<T2>() const;

	inline unsigned size() 		const 	{return this->std::vector<T>::size();}
	inline unsigned capacity() 	const 	{return this->std::vector<T>::capacity();}
	inline void clear()					{this->std::vector<T>::clear();}
	inline T* data() 					{return this->std::vector<T>::data();}//access to entries
	...
	Vec&& 	operator+ (const T&) &&;
	template <class T2> inline friend Vec<T2>   operator+ (const T2&, const Vec<T2>&);
	template <class T2> inline friend Vec<T2>&& operator+ (const T2&, Vec<T2>&&);
	template <class T2> inline friend Vec<T2>& operator+=(const T2&, Vec<T2>&);
	...
	inline Vec   operator^(const T& power) 		const&	{return pow(*this,power);}  //power of vector
	inline Vec&& operator^(const T& power) 		&&		{return std::move(pow((Vec&&)(*this),power));} 
	inline Vec   operator^(const Vec<T>& powers)const&	{return pow(*this,powers);}
	inline Vec&& operator^(Vec<T>&& powers) 	const&	{return std::move(pow(*this,(Vec&&)(powers)));}
	inline Vec&& operator^(const Vec<T>& powers)&&		{return std::move(pow((Vec&&)(*this),powers));}
	inline Vec&& operator^(Vec<T>&& powers) 	&&		{return std::move(pow((Vec&&)(*this),(Vec&&)(powers)));}
	...
	inline Vec&& join(Vec&& v)		const&	{return std::move((*this)&((Vec&&)v));}
	inline Vec&& join(const Vec& v)	&&		{return std::move(((Vec&&)(*this))&v);}
	inline Vec&& join(Vec&& v)		&&		{return std::move(((Vec&&)(*this))&((Vec&&)v));}

	inline Vec   mapV(T (*f)(T)) const&;
	inline Vec&& mapV(T (*f)(T)) &&;
	template <class T2> inline friend Vec<T2>   mapV(T2 (*f)(T2), const Vec<T2>& v);
	template <class T2> inline friend Vec<T2>&& mapV(T2 (*f)(T2), Vec<T2>&& v);
	template <class T2> inline Vec<T2> mapV(T2 (*f)(T)) const;
	template <class T2, class T3> friend Vec<T2> mapV(T2 (*f)(T3), const Vec<T3>& v);
	template <class T2> friend Vec<T2>   mapV2(T2 (*f)(T2,T2), const Vec<T2>& v1, const Vec<T2>& v2);
	template <class T2> friend Vec<T2>&& mapV2(T2 (*f)(T2,T2), Vec<T2>&& v1 	, const Vec<T2>& v2);
	template <class T2> friend Vec<T2>&& mapV2(T2 (*f)(T2,T2), const Vec<T2>& v1, Vec<T2>&& v2);
	template <class T2> friend Vec<T2>&& mapV2(T2 (*f)(T2,T2), Vec<T2>&& v1 	, Vec<T2>&& v2);
	...
	inline friend Vec&& sqrt (Vec&& v)		{return std::move(((Vec&&)v).mapV(sqrt));}
	inline friend Vec   fabs (const Vec& v)	{return v.mapV(fabs);}
	inline friend Vec&& fabs (Vec&& v)		{return std::move(((Vec&&)v).mapV(fabs));}
	...
	inline friend Vec&& cos  (Vec&& v)		{return std::move(((Vec&&)v).mapV(cos));}
	inline friend Vec   cosh (const Vec& v)	{return v.mapV(cosh);}
	inline friend Vec&& cosh (Vec&& v)		{return std::move(((Vec&&)v).mapV(cosh));}
	inline friend Vec   acos (const Vec& v)	{return v.mapV(acos);}
	inline friend Vec&& acos (Vec&& v)		{return std::move(((Vec&&)v).mapV(acos));}
	inline friend Vec   sin  (const Vec& v)	{return v.mapV(sin);}
	inline friend Vec&& sin  (Vec&& v)		{return std::move(((Vec&&)v).mapV(sin));}
	inline friend Vec   sinh (const Vec& v)	{return v.mapV(sinh);}
	inline friend Vec&& sinh (Vec&& v)		{return std::move(((Vec&&)v).mapV(sinh));}
	inline friend Vec   asin (const Vec& v)	{return v.mapV(asin);}
	inline friend Vec&& asin (Vec&& v)		{return std::move(((Vec&&)v).mapV(asin));}
	inline friend Vec   tan  (const Vec& v)	{return v.mapV(tan);}
	inline friend Vec&& tan  (Vec&& v)		{return std::move(((Vec&&)v).mapV(tan));}
	inline friend Vec   tanh (const Vec& v)	{return v.mapV(tanh);}
	inline friend Vec&& tanh (Vec&& v)		{return std::move(((Vec&&)v).mapV(tanh));}
	inline friend Vec   atan (const Vec& v)	{return v.mapV(atan);}
	inline friend Vec&& atan (Vec&& v)		{return std::move(((Vec&&)v).mapV(atan));}
	inline friend Vec   exp  (const Vec& v)	{return v.mapV(exp);}
	inline friend Vec&& exp  (Vec&& v)		{return std::move(((Vec&&)v).mapV(exp));}
	inline friend Vec   log  (const Vec& v)	{return v.mapV(log);}
	inline friend Vec&& log  (Vec&& v)		{return std::move(((Vec&&)v).mapV(log));}
	inline friend Vec   log10(const Vec& v)	{return v.mapV(log10);}
	inline friend Vec&& log10(Vec&& v)		{return std::move(((Vec&&)v).mapV(log10));}

	inline friend Vec   atan2(const Vec& v1, const Vec& v2)	{return mapV2(atan2,v1,v2);}
	inline friend Vec&& atan2(Vec&& v1 	 , const Vec& v2)	{return std::move(mapV2(atan2,((Vec&&)v1),v2));}
	inline friend Vec&& atan2(const Vec& v1, Vec&& v2)		{return std::move(mapV2(atan2,v1,((Vec&&)v2)));}
	inline friend Vec&& atan2(Vec&& v1 	 , Vec&& v2)		{return std::move(mapV2(atan2,((Vec&&)v1),((Vec&&)v2)));}

	inline friend Vec   pow(const Vec& v  , const T& power)		{return mapV2(pow,v,Vec(v.size(),power));}
	inline friend Vec&& pow(Vec&& v 	    , const T& power)	{return std::move(mapV2(pow,((Vec&&)v),Vec(v.size(),power)));}
	inline friend Vec   pow(const Vec& v1 , const Vec& v2)		{return mapV2(pow,v1,v2);}
	inline friend Vec&& pow(Vec&& v1 		, const Vec& v2)	{return std::move(mapV2(pow,((Vec&&)v1),v2));}
	inline friend Vec&& pow(const Vec& v1 , Vec&& v2)			{return std::move(mapV2(pow,v1,((Vec&&)v2)));}
	inline friend Vec&& pow(Vec&& v1 		, Vec&& v2)			{return std::move(mapV2(pow,((Vec&&)v1),((Vec&&)v2)));}
	inline friend Vec   pow(const T& base , const Vec& v)		{return mapV2(pow,Vec(v.size(),base),v);}
	inline friend Vec&& pow(const T& base , Vec&& v)			{return std::move(mapV2(pow,Vec(v.size(),base),((Vec&&)v)));}
	...
	inline friend std::ostream& operator<<(std::ostream& os, const Vec& v){v.print();return os;} //cout overloaded (example: std::cout << v1 << v2 << std::endl;)
	//print to file
	...
	inline Vec& insert(const T& x, int index=-1, unsigned places=1){return insert(Vec(places,x),index);} //insert at index 'index', 'places' new numbers with value 'x'
	...
	inline friend unsigned findMax(const Vec& v, int start=0, int end=-1){return v.findMax(start,end);}
	T max(int start=0, int end=-1) const{return (*this)[findMax(start,end)];} //maximum value
	inline friend T max(const Vec& v, int start=0, int end=-1){return v.max(start,end);}
	unsigned findMin(int start=0, int end=-1) const;
	inline friend unsigned findMin(const Vec& v, int start=0, int end=-1){return v.findMin(start,end);}
	T min(int start=0, int end=-1) const{return (*this)[findMin(start,end)];}
	inline friend T min(const Vec& v, int start=0, int end=-1){return v.min(start,end);}
	unsigned findZero(int start=0, int end=-1) const; //index of first TURNOVER point
	inline friend unsigned findZero(const Vec& v, int start=0, int end=-1){return v.findZero(start,end);}
	
	T norm(const unsigned type=2) const;
	template <class T2> inline friend T2 norm(const Vec<T2>& v, const unsigned type); //type=0 to infinity norm
	...
	inline friend Vec   sort(const Vec& v,bool down=false)	{return v.sort(down);}
	inline friend Vec&& sort(Vec&& v,bool down=false)		{return std::move(((Vec&&)v).sort(down));}
