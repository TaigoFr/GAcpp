...
public:
	...
	unsigned size() const 		{return this->std::vector<T>::size();}
	unsigned capacity() const 	{return this->std::vector<T>::capacity();}
	void clear(){this->std::vector<T>::clear();}
	T* data() {return this->std::vector<T>::data();}//access to entries
	const T* data() const {return this->std::vector<T>::data();}
	...
	// template <class T2> Vec<T2> mapV(T2 (*f)(T)) const;
	...
	friend Vec pow(const Vec& v, const T& power)	{return mapV2(pow,v,Vec(v.size(),power));}
