...
, numNonHidden(other.numNonHidden), numHidden(other.numHidden), numNodes(other.numNodes)
{
	...
	FOR(i, numOtherConnections) connections.push_back(new Connection(*other.connections[i]));
}
...
void Network::initialize(){
	FORV(i, numInputs+1){
		FOR(o, numInputs+1, numNonHidden)	addConnection(i, o, true);
	}
}
...
unsigned Network::countNode(unsigned n, bool countPre, bool countPos) const{
	unsigned count = 0;
	FORV(c, connections.size()){
		if(!connections[c]->getEnabled()) continue; //continue if disabled
		if((countPre && connections[c]->pre == n) || (countPos && connections[c]->pos == n))
			++count;
	}
	return count;
}
...
Network Network::crossover(const Network& n1, const Network& n2, double fit1, double fit2){
	if(n1.params == nullptr || n2.params == nullptr || n1.params != n2.params)
		errorMsg("Parent's parameters not equal.");
	if(n1.connections.size()==0 || n2.connections.size()==0)
		errorMsg("Parents with empty connections.");

	if(GA::generator() < n1.params->rateCrossoverAverage)
		return crossoverAverage(n1,n2,fit1,fit2);
	return crossoverNormal(n1,n2,fit1,fit2);
}
void Network::mutate(Network& child, const GA::Evolver<Network>*){
	...
	child.mutateChangeWeight();

	if(GA::generator() < child.params->mutateRateToggle)
		child.mutateToggle();
...
Network Network::crossoverAverage(const Network& n1, const Network& n2, double fit1, double fit2){
	unsigned iter1 = 0, inno1 = n1.connections[iter1]->innovationNumber, size1 = n1.connections.size();
	unsigned iter2 = 0, inno2 = n2.connections[iter2]->innovationNumber, size2 = n2.connections.size();
	...
	while(inno1 < last && inno2 < last){
		if(inno1==inno2){
			double save = n1.connections[iter1]->weight;
			n1.connections[iter1]->weight = (n1.connections[iter1]->weight + n2.connections[iter2]->weight)/2.;
			child.addConnection(n1.connections[iter1],
				(!n1.connections[iter1]->getEnabled() || !n2.connections[iter2]->getEnabled()) );
			n1.connections[iter1]->weight = save;
		}
		else if(inno1<inno2 && (fit1>fit2 || (fit1==fit2 && GA::generator()<0.5))){
			child.addConnection(n1.connections[iter1], n1.params->enableInExcessOrDisjoint);
		}
...
void Network::mutateChangeWeight(){
	FORV(i, connections.size()){
		if(!connections[i]->getEnabled()) continue; //continue if disabled
		if (GA::generator() < params->mutateRateWeightChange)
			connections[i]->weight = GA::generator(-params->mutateMaxChange, params->mutateMaxChange);
	}
}
bool Network::mutateToggle(){
	unsigned numConnections = getNumConnections();

	// unsigned con = GA::generator.getI(0, numConnections-1);
	unsigned con = 0;
	...
	unsigned count=0;
	while(countPos<3 || countPre<3 || pre==0){ //pre==0 is a bias
		if(count==numConnections) //no connection satisfied criteria
			return false;
		++count;

		con = (con+1)%numConnections;

		if(!connections[con]->getEnabled()) break;

		pre = connections[con]->pre;
		unsigned pos = connections[con]->pos;
		countPos = countNode(pos, false);
		countPre = countNode(pre, true, false);
	}

	connections[con]->toggle();
	return true;
}
...
double Network::dissimilarity(const Network& n1, const Network& n2){
	if(n1.params == nullptr || n2.params == nullptr || n1.params != n2.params)
		errorMsg("Parent's parameters NULL or not equal.");
	if(n1.connections.size()==0 || n2.connections.size()==0)
		errorMsg("Parents with empty connections.");
...
void Network::write(const std::string& path) const{
	...
	FORV(i, connections.size()) file << connections[i]->toString(true,10) << std::endl;
