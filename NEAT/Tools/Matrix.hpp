...
// operators
	...
	inline Matrix& removeRow(int start=-1, int end=-1){this->remove(start,end); return *this;}//erase row 'row'
	inline Matrix& removeCol(int start=-1, int end=-1){Matrix temp = transpose().remove(start,end); *this = temp.transpose(); return *this;}
	inline Matrix& eraseRow(int start=-1, int end=-1){return removeRow(start,end);}
	inline Matrix& eraseCol(int start=-1, int end=-1){return removeCol(start,end);}
